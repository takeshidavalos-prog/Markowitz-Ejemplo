...
# NOTA IMPORTANTE sobre qué se devuelve desde funciones con @st.cache_data:
#
# `st.cache_data` guarda el valor de retorno usando `pickle`. Streamlit
# vuelve a ejecutar TODO el script en cada interacción (cada movimiento de
# un slider, por ejemplo), lo que redefine las clases declaradas en este
# mismo archivo (`Estadisticos`, `ResultadoOptimizacion`, `FronteraEficiente`)
# como objetos "clase" nuevos en cada corrida. Si una corrida anterior aún
# está guardando en caché el resultado (p. ej. porque el cálculo de la
# frontera eficiente tarda un poco) justo cuando ya arrancó una corrida
# nueva, `pickle` puede fallar con:
#   PicklingError: Can't pickle <class '__main__.FronteraEficiente'>:
#   it's not the same object as __main__.FronteraEficiente
# porque la clase del objeto a guardar ya no es "la misma" que la que
# quedó registrada en el módulo tras la nueva corrida. Esto se ve en
# Streamlit Cloud (varias corridas superpuestas) aunque no se reproduzca
# en pruebas locales de una sola corrida.
#
# Para eliminar el problema de raíz, ninguna función cacheada devuelve
# instancias de esas clases: devuelven tuplas de tipos "estables" (pandas,
# numpy, listas, floats — que sí viven en módulos importados normalmente,
# no en este script) y el objeto de la clase correspondiente se reconstruye
# afuera de la caché, en cada corrida, donde nunca se serializa.
@st.cache_data(show_spinner=False)
def _estadisticos_cache(matriz_json: str, anualizar: bool):
    ...
    est = calcular_estadisticos(matriz, anualizar=anualizar)
    return est.mu, est.cov, est.corr, est.sigma, est.anualizado
...
def _gmv_cache(mu_tuple, cov_tuple, min_tuple, max_tuple):
    ...
    res = optimizar_min_varianza(mu, cov, lim_min, lim_max, retorno_objetivo=None)
    return res.exito, res.pesos, res.retorno, res.riesgo, res.mensaje
...
def _frontera_cache(mu_tuple, cov_tuple, min_tuple, max_tuple, n_puntos):
    ...
    lim_min = np.array(min_tuple)
    lim_max = np.array(max_tuple)
    resultado = calcular_frontera_eficiente(mu, cov, lim_min, lim_max, n_puntos=n_puntos)
...
_mu_est, _cov_est, _corr_est, _sigma_est, _anualizado_est = _estadisticos_cache(matriz_json, anualizar)
stats = Estadisticos(mu=_mu_est, cov=_cov_est, corr=_corr_est, sigma=_sigma_est, anualizado=_anualizado_est)
...
_gmv_exito, _gmv_pesos, _gmv_retorno, _gmv_riesgo, _gmv_mensaje = _gmv_cache(
    tuple(mu_arr), tuple(cov_arr.flatten()), tuple(limites_min), tuple(limites_max)
)
res_gmv_sidebar = ResultadoOptimizacion(_gmv_exito, _gmv_pesos, _gmv_retorno, _gmv_riesgo, _gmv_mensaje)
...
_frontera_raw = _frontera_cache(
    tuple(mu_arr), tuple(cov_arr.flatten()), tuple(limites_min), tuple(limites_max), n_puntos_frontera
)
frontera = FronteraEficiente(*_frontera_raw) if _frontera_raw is not None else None
